...
<div class="bg-amber-50 p-6 rounded-lg shadow-md border border-amber-200">
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-amber-800">Modifier la chambre</h2>
        
        <a href="{{ route('chambres.show', $chambre) }}" class="bg-amber-100 hover:bg-amber-200 text-amber-800 px-4 py-2 rounded-lg transition duration-200 flex items-center">
    ...
    <form action="{{ route('chambres.update', $chambre) }}" method="POST" class="bg-white p-6 rounded-lg shadow-sm border border-amber-100">
        @csrf
        @method('PUT')
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <input 
                        ...
                        name="superficie" 
                        id="superficie"
                        value="{{ old('superficie', $chambre->superficie) }}" 
                        class="w-full p-3 border border-amber-300 rounded-lg focus:ring-2 focus:ring-amber-500 focus:border-amber-500"
                    >
                </div>
                ...
                <div>
                    ...
                    >
                        @foreach (['RDC', '1', '2', '3'] as $etage)
                            <option value="{{ $etage }}" @selected(old('étage', $chambre->étage) == $etage)>
                                {{ $etage }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    >
                        @foreach ($types as $type)
                            <option value="{{ $type->id }}" @selected(old('type_id', $chambre->type_id) == $type->id)>
                                {{ $type->titre }}
                            </option>
                        @endforeach
                    </select>
